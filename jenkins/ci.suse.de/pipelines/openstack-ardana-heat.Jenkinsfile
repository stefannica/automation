    ...
    stage('Create heat stack') {
      ...
      steps {
        script {
          ...
          writeFile file: '$SHARED_WORKSPACE/heat_template.yml', text: params.heat_template

          retry(1) {
            lock(resource: 'cloud-ECP-API') {
              timeout(time: 10, unit: 'MINUTES') {
                ardana_lib.ansible_playbook('heat-stack', "-e heat_template_file=$SHARED_WORKSPACE/heat_template.yml")
              }
            }
          }
        }
